...
class TriggerRoutesTest {
    @Test
    fun `POST webhook with valid RawAlert returns 202 and alertId`() =
        testApplication {
    ...
    @Test
    fun `POST webhook with invalid payload returns 400 Bad Request`() =
        testApplication {
    ...
    @Test
    fun `POST slack with valid SlackTriggerRequest returns 202 Accepted`() =
        testApplication {
            val fakeIngestor = FakeAlertIngestor(shouldAccept = true)
            val testModule =
                module {
