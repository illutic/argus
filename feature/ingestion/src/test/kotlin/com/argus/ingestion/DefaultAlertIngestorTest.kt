...
class DefaultAlertIngestorTest {
    @Test
    fun `nominal webhook alert is accepted and enqueued`() =
        runBlocking {
    ...
    @Test
    fun `webhook alert preserves existing id if provided`() =
        runBlocking {
            val queue = ChannelAlertQueue(capacity = 10)
            val ingestor = DefaultAlertIngestor(queue)
    ...
    @Test
    fun `nominal slack trigger is accepted and converted to raw alert`() =
        runBlocking {
            val queue = ChannelAlertQueue(capacity = 10)
            val ingestor = DefaultAlertIngestor(queue)

            val result =
                ingestor.ingestSlack(
                    teamId = "core-team",
                    command = "/triage",
    ...
    @Test
    fun `webhook alert with blank teamId is rejected`() =
        runBlocking {
            val queue = ChannelAlertQueue(capacity = 10)
            val ingestor = DefaultAlertIngestor(queue)
    ...
    @Test
    fun `webhook alert with blank title is rejected`() =
        runBlocking {
            val queue = ChannelAlertQueue(capacity = 10)
            val ingestor = DefaultAlertIngestor(queue)
    ...
    @Test
    fun `slack trigger with blank teamId is rejected`() =
        runBlocking {
            val queue = ChannelAlertQueue(capacity = 10)
            val ingestor = DefaultAlertIngestor(queue)
    ...
    @Test
    fun `slack trigger with blank command is rejected`() =
        runBlocking {
            val queue = ChannelAlertQueue(capacity = 10)
            val ingestor = DefaultAlertIngestor(queue)
