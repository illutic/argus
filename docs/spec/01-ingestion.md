...
The Ingestion feature provides the entrypoint for all inbound alert triggers. It receives alert events via external HTTP
webhooks (Sentry, Humio, Datadog, Prometheus) and interactive Slack triggers (slash commands), normalizes them into a
canonical `RawAlert` data contract, and queues them for asynchronous triage processing.
...
- **Unit Tests**:
    ...
    - `TriggerRoutesTest`: Tests `202 Accepted` on valid webhook/slack payloads and `400 Bad Request` on malformed
      payloads.
