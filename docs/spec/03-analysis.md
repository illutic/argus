...
The Analysis feature evaluates the `EnrichedAlertContext` to produce an actionable `AlertDecision`. It combines
deterministic rule heuristics (spike threshold detection, deployment correlation) with generative LLM inference via a
local Ollama instance (`gpt-oss:20b`) to synthesize root-cause hypotheses, severity ratings, and recommended remediation
steps.
...
- `LlmTriageEngine`: Constructs structured triage prompt containing telemetry samples, git commits, flags, and logs;
  queries `LlmClient`; parses response into `AlertDecision`.
