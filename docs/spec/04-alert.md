...
The Alert feature delivers the final `AlertDecision` to designated operational channels (such as Slack, Jira issue
creation, or incident management webhooks) formatted as rich, actionable messages. For local environments, alerts are
routed to a console sink without making external network calls.
...
- `SlackAlertSink`: Formats decision into Slack Block Kit JSON and posts to team `slackChannelId` using Ktor HTTP
  client.
- `ConsoleAlertSink`: Prints formatted alert summary and root-cause hypothesis to standard logging/console for local
  development.
